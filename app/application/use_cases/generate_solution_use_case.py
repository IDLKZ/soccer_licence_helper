    ...
    def build_conclusion(
        ...
        if application_status_id == APPLICATION_STATUS_REVOKED_ID:
            return {
                ...
                1: f"«{club}» отказать в выдаче Лицензии на право участия в турнирах, организуемых {solution.type if solution.type else 'КФФ'} в сезоне «{season}» г.г.",
                2: f"Настоящее решение может быть обжаловано в Апелляционной комиссии по лицензированию футбольных клубов в порядке, определенном «Правилами по лицензированию футбольных клубов РК для участия в соревнованиях, организуемых {solution.type if solution.type else 'КФФ'}»."
            }

        if not articles:
            # Случай 1 — нет нарушений
            return {
                1: f"Выдать «{club}» Лицензию «{license}», организуемый {solution.type if solution.type else 'КФФ'} в сезоне «{season}» года.",
                2: f"Настоящее решение может быть обжаловано в Апелляционной комиссии по лицензированию футбольных клубов в порядке, определенном «Правилами по лицензированию футбольных клубов РК для участия в соревнованиях, организуемых {solution.type if solution.type else 'КФФ'}»."
            }
        ...
        return {
            ...
            2: sanctions_block,
            3: (
                f"Выдать «{club}» Лицензию «{license}», организуемый {solution.type if solution.type else 'КФФ'} в сезоне «{season}» года."
            ),
            4: (
            ...
            5: (
                f"Настоящее решение может быть обжаловано в Апелляционной комиссии по лицензированию футбольных клубов в порядке, определенном «Правилами по лицензированию футбольных клубов РК для участия в соревнованиях, организуемых {solution.type if solution.type else 'КФФ'}»."
            )
        }
